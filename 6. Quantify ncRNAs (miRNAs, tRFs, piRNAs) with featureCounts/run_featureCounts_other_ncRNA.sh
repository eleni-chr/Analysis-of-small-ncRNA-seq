...
output_dir="../featureCounts output other ncRNA"
mkdir -p "$output_dir"

# Path to the GTF file
gtf_file="../Bowtie index of other ncRNA reference/hsa_other_ncRNAs.gtf"
...
for sam_file in *_alignments.sam; do
    if [[ -f "$sam_file" ]]; then
        ...
        # Set output file names with "_other_ncRNA_counts" suffix
        output_txt="$output_dir/${base_name}_other_ncRNA_counts.txt"
        output_summary="$output_dir/${base_name}_other_ncRNA_counts.summary"
        ...
        echo "Processed: $sam_file -> ${base_name}_other_ncRNA_counts.txt and ${base_name}_other_ncRNA_counts.summary"
    fi
done

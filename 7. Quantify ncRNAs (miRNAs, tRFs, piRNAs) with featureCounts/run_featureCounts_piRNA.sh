...
output_dir="../featureCounts output piRNA"
mkdir -p "$output_dir"

# Path to the GTF file
gtf_file="../Bowtie index of piRNA reference/piRNA_gold_standard_set.gtf"
...
for sam_file in *_alignments.sam; do
    if [[ -f "$sam_file" ]]; then
        ...
        # Set output file names with "_piRNA_counts" suffix
        output_txt="$output_dir/${base_name}_piRNA_counts.txt"
        output_summary="$output_dir/${base_name}_piRNA_counts.summary"
        ...
        echo "Processed: $sam_file -> ${base_name}_piRNA_counts.txt and ${base_name}_piRNA_counts.summary"
    fi
done
